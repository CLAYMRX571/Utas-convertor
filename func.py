...
async def start_cmd(message: Message, state: FSMContext): 
    ...
    await message.answer( f"{text}\n\nQuyidagi tugmalardan birini tanlang 👇", reply_markup=menu) 
...
async def get_pdf_file(message: Message, state: FSMContext): 
    ...
    try: 
        ...
        await message.answer_document( FSInputFile(docx_path), caption="Mana Word faylingiz 📁", reply_markup=menu)
...
async def get_word_file(message: Message, state: FSMContext): 
    ...
    try: 
        ...
        await prog.edit_text("Tayyor ✅") 
        await message.answer_document( FSInputFile(pdf_path), caption="Mana PDF faylingiz 📁", reply_markup=menu ) 
    except Exception as e: 
        await message.answer(f"Xatolik yuz berdi: {e} ❌", reply_markup=menu) 
    finally: 
        await state.clear() 
...
async def rasm_menu(message: Message, state: FSMContext): 
    ...
    await message.answer("Rasm tashlang. Faqat JPG, JPEG yoki PNG tiniq formatda bo'lsin 📸", reply_markup=menu) 
...
async def translate_menu(message: Message, state: FSMContext): 
    ...
    await message.answer("Pdf yoki docx file tashlang 📁", reply_markup=menu) 
...
async def get_translate_file(message: Message, state: FSMContext): 
    ...
    if file_name.endswith(".pdf"): 
        ...
        await message.answer("Faqat pdf yoki docx file tashlang 📁", reply_markup=menu) 
        return await download_file(message.bot, doc.file_id, file_path) 
